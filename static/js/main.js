    ...
    if (post.awards && post.awards.length > 0) {
        ...
        if (badgeEl) {
            ...
            sortedAwards.forEach(aw => {
                ...
                if (awardCdStr.includes('P001A101') || awardNmStr.includes('슈퍼')) {
                    html += `<div class="winner-title-badge superstar-badge" style="position: relative; top: 0; right: 0; margin: 0; font-size: 1.1rem; padding: 0.5rem 1.15rem;"><i class="fa-solid fa-crown"></i> <span>슈퍼스타</span></div>`;
                } else if (awardCdStr.includes('P001A102') || awardNmStr.includes('라이징')) {
                    html += `<div class="winner-title-badge risingstar-badge" style="position: relative; top: 0; right: 0; margin: 0; font-size: 1.1rem; padding: 0.5rem 1.15rem;"><i class="fa-solid fa-wand-magic-sparkles"></i> <span>라이징스타</span></div>`;
                } else if (awardCdStr.includes('P001A103') || awardNmStr.includes('브라이트')) {
                    html += `<div class="winner-title-badge brightstar-badge" style="position: relative; top: 0; right: 0; margin: 0; font-size: 1.1rem; padding: 0.5rem 1.15rem;"><i class="fa-solid fa-star"></i> <span>브라이트스타</span></div>`;
                } else {
                    const rankSuffix = awRank ? ` ${awRank}위` : '';
                    html += `<div class="winner-title-badge family-badge" style="position: relative; top: 0; right: 0; margin: 0; font-size: 1.1rem; padding: 0.5rem 1.15rem;"><span class="pet-emoji-icon"><i class="${petIconClass}"></i></span> <span>패밀리스타${rankSuffix}</span></div>`;
                }
            });
            html += '</div>';
        ...
        if (badgeEl) {
            if (post.rank_candidate && !isClosedRound) {
                ...
                if (isFamily) {
                    badgeEl.innerHTML = `<div class="winner-title-badge family-badge" style="position: relative; top: 0; left: 0; right: auto; margin: 0; font-size: 1.15rem; padding: 0.55rem 1.25rem; font-weight: 800;"><span class="pet-emoji-icon"><i class="${petIconClass}"></i></span> <span>${rankTitle}</span></div>`;
                } else {
                    if (post.rank_candidate === 1) {
                        badgeEl.innerHTML = `<div class="rank-ribbon rank-1" style="font-size: 1.15rem; padding: 0.55rem 1.25rem; margin: 0; font-weight: 800;"><i class="fa-solid fa-medal"></i> ${rankTitle}</div>`;
                    } else if (post.rank_candidate === 2) {
                        badgeEl.innerHTML = `<div class="rank-ribbon rank-2" style="font-size: 1.15rem; padding: 0.55rem 1.25rem; margin: 0; font-weight: 800;"><i class="fa-solid fa-medal"></i> ${rankTitle}</div>`;
                    } else if (post.rank_candidate === 3) {
                        badgeEl.innerHTML = `<div class="rank-ribbon rank-3" style="font-size: 1.15rem; padding: 0.55rem 1.25rem; margin: 0; font-weight: 800;"><i class="fa-solid fa-medal"></i> ${rankTitle}</div>`;
                    } else {
                        badgeEl.innerHTML = '';
                    }
                }
