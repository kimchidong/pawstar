...
class PawStarService:
    ...
    def is_nickname_taken(self, nickname, exclude_user_id=None, conn=None):
        ...
        # 항시 전용 독립 커넥션으로 조회하여 트랜잭션 충돌 및 오탐지 완벽 방지
                ...
                try:
                    ...
                    if cur.fetchone():
                        if ex_id:
                            cur.execute("""
                                SELECT 1 FROM USERS 
                                WHERE LOWER(TRIM(NICKNAME)) = LOWER(%s) 
                                  AND LOWER(TRIM(USER_ID)) != LOWER(%s)
                                LIMIT 1
                            """, (nk, ex_id))
                        else:
                            cur.execute("SELECT 1 FROM USERS WHERE LOWER(TRIM(NICKNAME)) = LOWER(%s) LIMIT 1", (nk,))
                        if cur.fetchone():
                            check_conn.close()
                            return True
                except Exception:
            ...
            with conn.cursor() as cur:
                ...
                award_query = """
                    SELECT 
                        ...
                        r.VW_CNT AS view_count, r.LIKE_CNT AS like_count, r.CMT_CNT AS comment_count, COALESCE(r.SHARE_CNT, 0) AS share_count, r.SHARE_SN AS share_sn,
                        r.VW_CNT, r.LIKE_CNT, r.CMT_CNT, COALESCE(r.SHARE_CNT, 0) AS SHARE_CNT, r.SHARE_SN AS SHARE_SN,
                ...
                if contest_id and str(contest_id) != 'all':
                    ...
                    award_params.append(str(contest_id))
            ...
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT 
                        ...
                        ca.CMT_CNT,
                        COALESCE(r.SHARE_CNT, 0) AS SHARE_CNT,
                        COALESCE(r.SHARE_SN, '') AS SHARE_SN,
